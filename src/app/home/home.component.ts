...
export class HomeComponent implements OnInit {
  ...
  testModel() {
    ...
    this.userService.testModel(this.selectedModel, dataNodes, this.predicateMatrix).pipe(first()).subscribe(
      res => {
        ...
        console.log(n);
        if (res)
          this.showSuccess("Model predicted! Result: " + res);
        else
          this.showSuccess("Model predicted!");
      },
      err => {
        console.log("Error occured");
        this.showError("Model prediction failed!");
      }
    );
  }
  ...
  trainBinaryPerceptron() {
    this.userService.trainBinaryPerceptron(this.selectedModel).pipe(first()).subscribe(result => {
      console.log(result);
      this.showSuccess("Model trained succesfully! <br>" + JSON.stringify(result));
    },
      err => {
        console.log("Error occured!");
        this.showError("Model train failed!");
      });
  }
  ...
  littleGraph(modelName: any) {
    ...
    var config = {
      ...
      relationships: {
        "related": {
          "caption": "related",
          "thickness": "kernel" //kernel, weight, bias
        }
      },
    ...
    setTimeout(() => {
      if (this.exportCypherEl && this.exportCypherEl.nativeElement.classList.contains('hide')) {
        ...
        this.userService.createModel(this.exportCypherModel).subscribe(
          res => {
            console.log(res);
            this.showSuccess("Model created succesfully!");
            this.exportCypherEl.nativeElement.classList.add("hide");
          },
          err => {
            console.log("Error occured");
            this.showError("Model could not created!");
          }
        );
      }
    }, 300);
